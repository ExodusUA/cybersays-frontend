...
function Message({ setOpenMassege, message, copyToMessage, messageCopied, onCloseCopied, selectedMessage  }) {
            ...
            <div className='max-w-[600px] w-full m-auto'>
                <div className='flex justify-end mb-4 md:mb-10'>
                    <img onClick={e => {
                        ...
                        }} className='w-[24px] h-[24px] cursor-pointer' src={close} alt="close" />
                </div>
                <div className='my-6'>
                    <p className='h-auto max-h-[400px] md:max-h-[650px] saira text-[14px] font-medium  text-center overflow-y-scroll'>{message}</p>
                </div> 

                <button onClick={e => copyToMessage(message, selectedMessage)} disabled={messageCopied[selectedMessage]} className='max-w-[350px] m-auto w-full bg-white  border-[2px] border-[#FFED63] rounded-[50px] text-black text-[18px] saira font-semibold py-2 disabled:opacity-60 flex justify-center items-center'>
                    Copy message
