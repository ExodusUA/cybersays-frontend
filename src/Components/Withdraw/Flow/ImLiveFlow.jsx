...
function ImLiveFlow({ languageData, setConfirm, setFlowStarted, setError }) {
            ...
            <div className=' m-auto max-w-[600px] w-full'>
                <div className={` backdrop-blur-xl bg-opacity-60 p-4 bg-black rounded-[20px] ${design === '0' ? ' border-[#FFD700]' : '  border-[#A2DBF0]'} border`}>
                    ...
                    <div className='flex items-center gap-4'>
                        <button onClick={e => setFlowStarted(false)} className={`w-full bg-transparent  border px-6 py-2 saira p-2 mt-4 max-w-[350px] text-[18px] text-white font-semibold ${design === '0' ? '  rounded-[50px] border-[#FFD700]' : ' rounded-[12px] border-[#A2DBF0]'}`}>{languageData?.logoutConfirmNo}</button>
                        <button onClick={e => handleImLiveFlow()} className={`w-full bg-[white]  px-6 py-2 saira p-2 mt-4 max-w-[350px] text-[18px] text-[#5f5f5f] font-semibold ${design === '0' ? '  rounded-[50px] border-[2px] bg-white ' : ' rounded-[12px] border-none gradient-homepageBtn'}`}>{languageData?.logoutConfirmYes}</button>
                    </div>
                </div>
            </div>
        </div>
